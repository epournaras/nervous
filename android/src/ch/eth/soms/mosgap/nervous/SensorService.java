...
public class SensorService extends Service implements SensorEventListener {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		sensorTemperature = sensorManager
				.getDefaultSensor(Sensor.TYPE_AMBIENT_TEMPERATURE);
		sensorHumidity = sensorManager
				.getDefaultSensor(Sensor.TYPE_RELATIVE_HUMIDITY);

		hasAccelerometer = sensorManager.registerListener(this,
				sensorAccelerometer, SensorManager.SENSOR_DELAY_NORMAL);
		hasLight = sensorManager.registerListener(this, sensorLight,
				SensorManager.SENSOR_DELAY_NORMAL);
		hasMagnet = sensorManager.registerListener(this, sensorMagnet,
				SensorManager.SENSOR_DELAY_NORMAL);
		hasProximity = sensorManager.registerListener(this, sensorProximity,
				SensorManager.SENSOR_DELAY_NORMAL);
		hasGyroscope = sensorManager.registerListener(this, sensorGyroscope,
				SensorManager.SENSOR_DELAY_NORMAL);
		hasTemperature = sensorManager.registerListener(this,
				sensorTemperature, SensorManager.SENSOR_DELAY_NORMAL);
		hasHumidity = sensorManager.registerListener(this, sensorHumidity,
				SensorManager.SENSOR_DELAY_NORMAL);

		sensorHeader = new SensorHeader(sensorManager);

		// True means the sensor works with a listener and we don't know when it
		// will be triggered
		if (hasAccelerometer) {
		...
		// False means it's a parameter we can query, therefore we don't have to
		// wait for it
	...
	public void onSensorChanged(SensorEvent event) {
		...
		case Sensor.TYPE_LIGHT:
			sensorFrame.addSensorData(new SensorDataLight(timestamp,
					event.accuracy, event.values[0]));
			Log.d(DEBUG_TAG, "Light data added to frame");
			break;
		case Sensor.TYPE_PROXIMITY:
			sensorFrame.addSensorData(new SensorDataProximity(timestamp,
					event.accuracy, event.values[0]));
			Log.d(DEBUG_TAG, "Proximity data added to frame");
			break;
		case Sensor.TYPE_ACCELEROMETER:
			sensorFrame.addSensorData(new SensorDataAccelerometer(timestamp,
					event.accuracy, event.values[0], event.values[1],
					event.values[2]));
			Log.d(DEBUG_TAG, "Accelerometer data added to frame");
			break;
		case Sensor.TYPE_MAGNETIC_FIELD:
			sensorFrame.addSensorData(new SensorDataMagnetic(timestamp,
					event.accuracy, event.values[0], event.values[1],
					event.values[2]));
			Log.d(DEBUG_TAG, "Magnetic data added to frame");
			break;
		case Sensor.TYPE_GYROSCOPE:
			sensorFrame.addSensorData(new SensorDataGyroscope(timestamp,
					event.accuracy, event.values[0], event.values[1],
					event.values[2]));
			Log.d(DEBUG_TAG, "Gyroscope data added to frame");
			break;
		case Sensor.TYPE_AMBIENT_TEMPERATURE:
			sensorFrame.addSensorData(new SensorDataTemperature(timestamp,
					event.accuracy, event.values[0]));
			Log.d(DEBUG_TAG, "Temperature data added to frame");
			break;
		case Sensor.TYPE_RELATIVE_HUMIDITY:
			sensorFrame.addSensorData(new SensorDataHumidity(timestamp,
					event.accuracy, event.values[0]));
		...
		if (sensorFrame.isComplete()) {
			...
			SensorDataBattery sensorDataBattey = new SensorDataBattery(
					timestamp, batteryPct, isCharging, usbCharge, acCharge);
	 ...
	 */
	private class SensorServiceLoggerTask extends
			AsyncTask<SensorFrame, Void, Void> {

		@Override
		protected Void doInBackground(SensorFrame... frames) {
			writeToSdCard(frames);
			// originalWriteToSDCard(frames);
			return null;
		}
		...
		private void originalWriteToSDCard(SensorFrame... frames) {
			...
			try {
				...
				new ServiceInfo(getApplicationContext()).setFileSize(file
						.length());
		...
		/**
		 * Used to write the file on the SD Card for easy access
		 * 
		 * @param frames
		 */
		private void writeToSdCard(SensorFrame... frames) {
			// TODO: use originalWriteToSDCard so files cannot be easily
			// manipulated by the user

			Log.d(DEBUG_TAG, "----------Writing extra File");
			...
			try {
				...
				dir.mkdirs();
				String file_name = "SensorLog1.txt";
